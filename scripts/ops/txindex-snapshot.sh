...
asset_issuance="$(docker exec "$verify_container" raven-cli -datadir=/data -conf=/etc/ravencoin/raven.conf getrawtransaction "$asset_probe_txid" true)"
actual_asset_block="$(printf '%s' "$asset_issuance" | jq -r '.blockhash // empty')"
[[ "$actual_asset_block" == "$asset_probe_block" ]] || { echo "Staged asset issuance transaction is not in the expected block." >&2; exit 1; }
asset_data="$(docker exec "$verify_container" raven-cli -datadir=/data -conf=/etc/ravencoin/raven.conf getassetdata "$asset_probe_name")"
