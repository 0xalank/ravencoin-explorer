...
CREATE INDEX IF NOT EXISTS transactions_time_idx ON transactions (time DESC);
...
CREATE INDEX IF NOT EXISTS tx_outputs_unspent_idx ON tx_outputs (txid, vout_index) WHERE spent_by_txid IS NULL;
CREATE INDEX IF NOT EXISTS tx_outputs_asset_idx ON tx_outputs (asset_name) WHERE asset_name IS NOT NULL;
...
CREATE INDEX IF NOT EXISTS tx_inputs_previous_idx ON tx_inputs (prev_txid, prev_vout);
...
CREATE INDEX IF NOT EXISTS address_activity_history_idx ON address_activity (address, block_height DESC, tx_index DESC);
CREATE INDEX IF NOT EXISTS address_activity_asset_idx ON address_activity (address, asset_name);
